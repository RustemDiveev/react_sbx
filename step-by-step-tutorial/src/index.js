...
/************************** Думать в парадигме React ******************************/

/*
    Необходимо начать с грязного прототипа и исходить из того, что уже есть JSON API, возвращающее данные и макет дизайнера.

    Шаг 1: Разбить UI на иерархию компонентов 
    Принцип одиночной ответственности - один компонент в идеале должен делать одну вещь 
    Эту информацию, может также и предоставить корректное JSON API. 
    1. Определить конечный состав компонентов 
    2. Продумать иерархию 

    Шаг 2: Построить статическую версию на React 
    Т.е просто отрисовать UI, но пока не добавлять интерактивности.
    Статическая версия требует много печатать, но мало думать, а динамика - наоборот.
    Использовать пока что props, но не использовать state.

    Шаг 3: Идентифицировать минимальное, но законченное представление состояния UI.
    Т.е первоначальное состояние State, и где оно нужно.
    Минимальный набор мутабельного состояния, в котором нуждается приложение.

    Шаг 4: Идентфицировать, где должно жить состояние
    Опозать, какой компонент изменяет (владеет) этим состоянием.

    Для каждого куска состояния приложения:
    1. Идентифицировать каждый компонент, который отрисовывает что либо, базируясь от этого состояния.
    2. Найти общий компонент-владелец 
    3. Либо этот компонент из п.2 либо компонент выше должны владеть этим состоянием.
    4. Если не получается определить такой компонент, то создать с нуля и определить его по иерархии между компонентами из п.2 и п.1

    Шаг 5: Добавить обратгный поток данных 
    Самые глубокие в иерархии компоненты должны обновлять состояние самого верхнеуровневого компонента.
*/